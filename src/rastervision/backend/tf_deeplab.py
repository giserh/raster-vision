...
def get_export_args(export_model_py: str, train_logdir_local: str,
                    num_classes: int, tfdl_config) -> List[str]:
...
class TFDeeplab(Backend):
    ...
    def process_scene_data(self, scene: Scene, data: TrainingData,
        ...
        base_uri = self.config.training_data_uri
    ...
    def process_sceneset_results(self, training_results: List[str],
        ...
        if self.config.debug:
            ...
            with tempfile.TemporaryDirectory() as debug_dir:
                make_debug_images(
                    training_record_path_local, debug_dir, self.class_map,
                    self.task_options.chip_options.debug_chip_probability)
                shutil.make_archive(training_zip_path_local, 'zip', debug_dir)
            with tempfile.TemporaryDirectory() as debug_dir:
                make_debug_images(
                    validation_record_path_local, debug_dir, self.class_map,
                    self.task_options.chip_options.debug_chip_probability)
    ...
    def train(self, tmp_dir: str) -> None:
        ...
        Returns:
             None
        """
        train_py = self.backend_options.script_locations.train_py
        export_model_py = self.backend_options.script_locations.export_model_py

        # Restart support
        train_restart_dir = self.backend_options.train_options.train_restart_dir
        ...
        train_logdir = self.backend_options.training_output_uri
        train_logdir_local = get_local_path(train_logdir, tmp_dir)
        dataset_dir = self.backend_options.training_data_uri
        ...
        # Setup TFDL config
        tfdl_config = json_format.ParseDict(self.backend_config.tfdl_config,
                                            TrainingParametersMsg())
        print('tfdl_config={}'.format(tfdl_config))
        print('Training steps={}'.format(tfdl_config.training_number_of_steps))

        # Additional training options
        max_class = max(list(map(lambda c: c.id, self.class_map.get_items())))
    ...
    def predict(self, chips: np.ndarray, windows: List[Box],
                tmp_dir) -> List[Tuple[Box, np.ndarray]]:
